...
import { VisualTimeline, Node, GeneratorNode, EffectNode, ControlParam } from './nodes/types.js';
 ...
 */
function analyzeMusicStructure(features: DeepAudioFeatures): MusicStructure {
  const { duration, beats, rms, tempo } = features;

  // Detect energy changes (often indicate section changes)
  const sectionChanges: number[] = [0];
 ...
 */
function parseTimelineFromResponse(response: string, duration: number): VisualTimeline {
 ...
 */
export function generateTestTimeline(features: DeepAudioFeatures): VisualTimeline {
  ...
  timeline.nodes.push({
    ...
    effect: {
      name: 'bloom',
      threshold: { type: 'static', value: 0.5 } as ControlParam,
      intensity: {
        type: 'audio-reactive',
        audioSource: { type: 'beat', confidence: 0.7 },
        audioRange: [0.5, 2.5],
