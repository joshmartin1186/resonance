 ...
 */
export async function uploadVideoWithFallbacks(
  outputPath: string,
  ...
  throw new Error(
    `All upload methods failed. Video saved to: ${savedPath}. ` +
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        const upload = new tus.Upload(videoBuffer, {
          ...
          onError: (error) => {
            ...
            const retryAttempt = (upload as any)._retryAttempt || 0
            if (retryAttempt >= 7) {
              reject(error)
            }
          },
          ...
          onProgress: (bytesUploaded, bytesTotal) => {
            ...
            console.log(
              `[TUS] Progress: ${percentage}% (${uploadedMB}/${totalMB}MB) @ ${speedMBps}MB/s`
            )
 ...
 */
async function uploadViaStandard(
  outputPath: string,
  videoFileName: string,
  stats: { size: number }
): Promise<string> {
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      if (contentLength) {
        ...
        if (variance > 0.02) {
          throw new Error(
            `Size mismatch: expected ${expectedSize} bytes, got ${uploadedSize} bytes (${(variance * 100).toFixed(1)}% diff)`
          )
        }
      }
 ...
 */
async function saveForRecovery(
  outputPath: string,
  projectId: string,
  videoFileName: string,
  error: Error | null
