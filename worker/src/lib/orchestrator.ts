 ...
 */
function buildUserPrompt(input: OrchestrationInput): string {
 ...
 */
function findPeaks(energyCurve: number[]): number[] {
  const peaks: number[] = []
  const threshold = 0.7
 ...
 */
function parseVisualPlan(text: string): VisualPlan {
  ...
  try {
    return JSON.parse(jsonStr) as VisualPlan
  } catch (err) {
 ...
 */
function generateFallbackPlan(input: OrchestrationInput): VisualPlan {
  ...
  const segments: VisualSegment[] = sections.length > 0
...
function createTimeBasedSegments(duration: number, style: string, footageCount: number): VisualSegment[] {
...
function getEffectsForTone(tone: string, style: string, energy: number): EffectAssignment[] {
...
function getColorPaletteForStyle(style: string): VisualPlan['colorPalette'] {
  switch (style) {
    case 'psychedelic':
...
function getGlobalEffectsForStyle(style: string): EffectAssignment[] {
  switch (style) {
    case 'cinematic':
      return [
