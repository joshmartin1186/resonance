...
export interface DeepAudioFeatures {
  ...
  // Time-series data (30fps arrays)
  rms: number[];              // Root mean square (overall energy)
  zcr: number[];              // Zero crossing rate (texture/noise)
  spectralCentroid: number[]; // Brightness
  spectralRolloff: number[];  // High frequency content
  spectralFlux: number[];     // Rate of change (useful for onsets)

  // Frequency bands (30fps)
  ...
  // Advanced features
  mfcc: number[][];           // Mel-frequency cepstral coefficients (timbre) - 13 coefficients per frame
  chroma: number[][];         // Pitch class (12 per frame)
  ...
  energy: number[];           // Overall energy curve
}
 ...
 */
export async function analyzeAudioDeep(audioPath: string): Promise<DeepAudioFeatures> {
 ...
 */
async function getAudioMetadata(audioPath: string): Promise<{ duration: number; sampleRate: number }> {
 ...
 */
async function decodeAudioToPCM(audioPath: string, targetSampleRate: number): Promise<number[]> {
 ...
 */
function extractFrequencyBands(frame: Float32Array, sampleRate: number): {
  bass: number;
  lowMid: number;
  mid: number;
  highMid: number;
 ...
 */
function calculateBandEnergy(frame: Float32Array, freqStart: number, freqEnd: number): number {
  let energy = 0;
 ...
 */
function detectBeats(spectralFlux: number[], fps: number): Beat[] {
  const beats: Beat[] = [];
 ...
 */
function estimateTempo(beats: Beat[], duration: number): number {
 ...
 */
export function normalizeFeature(values: number[]): number[] {
