...
export interface ParallelRenderOptions {
  ...
  height: number;
  fps: number;
  workerCount?: number; // Default: CPU count - 1
 ...
 */
export async function renderTimelineParallel(options: ParallelRenderOptions): Promise<void> {
  const {
    timeline,
    audioFeatures,
    outputDir,
  ...
  const chunks: { start: number; end: number; workerId: number }[] = [];
 ...
 */
export function encodeFramesToVideo(
  framesDir: string,
  audioPath: string,
  outputPath: string,
  fps: number
