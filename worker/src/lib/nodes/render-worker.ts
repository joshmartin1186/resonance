...
async function renderChunk(
  chunk: { start: number; end: number; workerId: number },
  ...
  for (let frame = start; frame < end; frame++) {
    ...
    const activeGenerators = timeline.nodes
      ...
      .filter(node => calculateNodeOpacity(node, time) > 0) as GeneratorNode[];
    ...
    const activeEffects = timeline.nodes
      ...
      .filter(node => calculateNodeOpacity(node, time) > 0) as EffectNode[];
...
function createFramebuffer(gl: any, width: number, height: number) {
...
function compositeTexture(gl: any, srcTexture: any, dstFramebuffer: any, opacity: number, width: number, height: number) {
  const vertexShader = `
...
function blendTextures(gl: any, texture1: any, texture2: any, blend: number, width: number, height: number) {
  const vertexShader = `
    attribute vec2 position;
    varying vec2 vUv;
    void main() {
