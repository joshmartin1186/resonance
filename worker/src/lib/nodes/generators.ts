...
import { GeneratorNode, NodeRenderContext, evaluateParam, ControlParam } from './types.js';
 ...
 */
export function renderPerlinNoise(
  node: GeneratorNode,
  ctx: NodeRenderContext
): void {
  const { gl, time, width, height, audioFeatures, audioFrame } = ctx;
  const gen = node.generator as any;
 ...
 */
export function renderRadialWaves(
  node: GeneratorNode,
  ctx: NodeRenderContext
): void {
  const { gl, time, width, height, audioFeatures, audioFrame } = ctx;
 ...
 */
export function renderSolidColor(
  node: GeneratorNode,
  ctx: NodeRenderContext
): void {
  const { gl } = ctx;
 ...
 */
export function renderGenerator(node: GeneratorNode, ctx: NodeRenderContext): void {
  const genName = (node.generator as any).name;
