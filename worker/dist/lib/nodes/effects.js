 ...
 * Effect 1: Blur
 */
export function renderBlur(node, ctx) {
 ...
 */
export function renderBloom(node, ctx) {
    const { gl, time, width, height, audioFeatures, audioFrame, inputBuffer } = ctx;
    const effect = node.effect;
 ...
 */
export function renderKaleidoscope(node, ctx) {
    const { gl, time, width, height, audioFeatures, audioFrame, inputBuffer } = ctx;
    const effect = node.effect;
 ...
 */
export function renderColorGrade(node, ctx) {
    const { gl, time, width, height, audioFeatures, audioFrame, inputBuffer } = ctx;
    const effect = node.effect;
 ...
 */
export function renderFeedback(node, ctx) {
    const { gl, time, width, height, audioFeatures, audioFrame, inputBuffer } = ctx;
    const effect = node.effect;
 ...
 */
export function renderChromaticAberration(node, ctx) {
 ...
 * Effect: Film grain
 */
export function renderGrain(node, ctx) {
    const { gl, time, width, height, audioFeatures, audioFrame } = ctx;
    const effect = node.effect;
    const amount = evaluateParam(effect.amount, time, audioFeatures.duration, audioFeatures, audioFrame);
    // Film grain effect
 ...
 */
export function renderEffect(node, ctx) {
    const effectName = node.effect.name;
    switch (effectName) {
        case 'blur':
            renderBlur(node, ctx);
            break;
