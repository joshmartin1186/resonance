 ...
 */
export async function runFFmpeg(options, onProgress) {
    const { inputPath, outputPath, filters, duration, startTime, audioPath, resolution = '1080p', fps = 30, codec = 'h264', quality = 23 } = options;
 ...
 */
export function buildFilterChain(effects) {
 ...
 */
export async function imageToVideo(imagePath, outputPath, duration, options = {}) {
    const args = [
        '-y',
        '-loop', '1',
        '-i', imagePath,
