 ...
 */
export function loadShader(type) {
 ...
 */
export function getAudioFeaturesAtTime(audioFeatures, time) {
 ...
 */
function sampleCurve(curve, normalizedTime) {
    if (curve.length === 0)
        return 0.5;
 ...
 */
function detectTransient(curve, normalizedTime) {
    if (curve.length < 3)
        return 0;
    const index = Math.floor(normalizedTime * (curve.length - 1));
 ...
 */
export function buildShaderFilter(config, resolution) {
 ...
 */
function buildApproximateFilter(type, duration, audioFeatures, colors, intensity, width, height) {
 ...
 * Get shader metadata
 */
export function getShaderInfo(type) {
    const shaderInfo = {
        'perlin-noise': {
