...
import { buildShaderFilter } from './shader-manager.js';
// Temp directory for rendering
const TEMP_DIR = '/tmp/resonance-render';
/**
 * Main rendering pipeline
 */
export async function renderVideo(config, visualPlan, onProgress) {
    ...
    try {
        ...
        for (let i = 0; i < visualPlan.segments.length; i++) {
            ...
            if (segment.footageIndex !== undefined && footagePaths[segment.footageIndex]) {
                ...
                await processFootageSegment(footagePaths[segment.footageIndex], segmentPath, segment, config);
            }
            else if (segment.generativeType || segment.shaderType) {
                // Generate procedural content with shaders or legacy generative patterns
                await generateProceduralSegment(segmentPath, segment, config, visualPlan.colorPalette);
            }
            else {
                // Black/solid color segment
                await createSolidSegment(segmentPath, segment, visualPlan.colorPalette[0] || '#000000', config);
            }
 ...
 */
async function processFootageSegment(inputPath, outputPath, segment, config) {
    // Build filter chain from effects
    const filters = buildFilterChain(segment.effects.map(effect => ({
 ...
 */
async function generateProceduralSegment(outputPath, segment, config, colorPalette = ['#C45D3A', '#2A2621', '#F0EDE8']) {
 ...
 */
async function createSolidSegment(outputPath, segment, color, config) {
    const duration = segment.endTime - segment.startTime;
 ...
 */
export function generateVisualPlan(audioFeatures, prompt, style, footageCount) {
...
function getDefaultEffects(style) {
    const effects = [];
    switch (style) {
        case 'cinematic':
            effects.push({
...
function getEffectsForSection(section, style) {
...
function getGenerativeForSection(section) {
    if (section.energy > 0.7) {
        return 'particles';
    }
...
function getMoodFromFeatures(features) {
 ...
 */
export function cleanupRender(projectId) {
